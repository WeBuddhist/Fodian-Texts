...
# The Buddha as author means buddhavacana — a sutra, not an authored
# treatise. Both spellings appear in the sheet.
BUDDHA_AUTHORS = {"སངས་རྒྱས་བཅོམ་ལྡན་འདས།", "佛世尊", "buddha bhagavan"}

# Title markers that identify an authored text as a commentary on another.
COMMENTARY_EN = re.compile(r"\bcommentary\b|\bexplanation of\b|\bthe meaning of\b", re.I)
COMMENTARY_ZH = re.compile(r"註釋|注釋|釋論|難語釋|之義|[^\w]釋$|釋$")


COMMENTARY_SUFFIX_ZH = re.compile(r"(註釋|注釋|釋論|難語釋|之義|釋)$")


def find_commentary_target(zh_title, candidates):
    """Match a commentary to the text it comments on, conservatively.

    Only an exact match after stripping the Chinese commentary suffix counts
    — `三身讚註釋` minus `註釋` is `三身讚`, which is a title in this corpus.
    Anything looser (substring overlap, English paraphrase) would pair texts
    that merely share vocabulary, so those are left for a human instead.
    """
    t = (zh_title or "").strip()
    if not t:
        return None
    stripped = COMMENTARY_SUFFIX_ZH.sub("", t).strip()
    if stripped and stripped != t and stripped in candidates:
        return candidates[stripped]
    return None


def classify_work(rec_bo, rec_zh=None):
    """Return (work_type, evidence).

    Evidence comes from the sheet only: the author field separates
    buddhavacana from authored treatise, and the title says whether an
    authored treatise comments on something. The sheet's *category* column
    cannot be used for this — `4.1 唯識宗阿毗達磨釋論` ("Yogācāra Abhidharma
    commentarial treatises") contains both Asaṅga's Abhidharmasamuccaya,
    which is a root treatise, and Jinaputra's commentary on it.
    """
    author = (rec_bo.get("author_orig") or "").strip()
    author_en = (rec_bo.get("author_en") or "").strip().lower()
    if author in BUDDHA_AUTHORS or author_en in BUDDHA_AUTHORS:
        return "root-text", "author is the Buddha (buddhavacana)"

    en = rec_bo.get("name_en") or ""
    zh = ((rec_zh or {}).get("name_short") or "") + " " + ((rec_zh or {}).get("name_long") or "")
    if COMMENTARY_EN.search(en):
        return "commentary", "English title: %r" % en[:60]
    if COMMENTARY_ZH.search(zh.strip()):
        return "commentary", "Chinese title: %r" % zh.strip()[:40]
    return "treatise", "authored (%s), title does not mark it as a commentary" % (
        rec_bo.get("author_en") or author or "unknown")
...
def write_inventory(rows, path, today):
    """A single page answering: what is in 1-SOURCES, and how does it relate?"""
    bo = [r for r in rows if r["tag"] == "bo"]
    zh = [r for r in rows if r["tag"] != "bo"]
    by_type = collections.Counter(r["work_type"] for r in bo)
    by_reg = collections.Counter(r["register"] for r in zh)

    out = ["# 1-SOURCES inventory", "",
           "Generated by the `inbox-to-sources` skill on %s. Regenerate rather than edit." % today,
           "", "## Totals", "",
           "| | count | folder |", "|---|---|---|",
           "| Root texts (buddhavacana) | %d | `Text/` |" % by_type.get("root-text", 0),
           "| Treatises (authored, not commenting on another text) | %d | `Text/` |" % by_type.get("treatise", 0),
           "| Commentaries | %d | `Commentaries/` |" % by_type.get("commentary", 0),
           "| **Tibetan witnesses total** | **%d** | |" % len(bo),
           "", "| Chinese witness | count | folder |", "|---|---|---|",
           "| Literary Chinese (`zh-hant`) | %d | `Translations/` or `Commentaries/` |" % by_reg.get("literary", 0),
           "| Modern Chinese (`zh-modern`) | %d | `Translations/` |" % by_reg.get("modern", 0),
           "| **Chinese witnesses total** | **%d** | |" % len(zh),
           "",
    ...
    coms = [r for r in bo if r["work_type"] == "commentary"]
    if coms:
        out += ["| Commentary | Title | Comments on | In this corpus? |", "|---|---|---|---|"]
        for r in sorted(coms, key=lambda x: x["text"]):
            tgt = r.get("commentary_on")
            out.append("| `%s` | %s | %s | %s |" % (
                r["text"], (r.get("title_en") or r["title"])[:56],
                ("`%s`" % tgt) if tgt else "_not identified automatically_",
                "yes" if tgt else "needs a human check"))
        out += ["",
    ...
    for r in zh:
        bykey[r["text"]].append(r)
    for r in sorted(bo, key=lambda x: x["text"]):
        zs = bykey.get(r["text"], [])
        out.append("| `%s` | %s | `%s/%s` | %s | %d |" % (
            r["text"], r["work_type"], r["folder"], r["file"],
            ", ".join("%s (%s)" % (z["tag"], z["register"]) for z in zs) or "—",
            r["segments"]))
    out.append("")
...
def stage_write(plan, records, manifest, vault="."):
    ...
    comdir = os.path.join(vault, "1-SOURCES", "Commentaries")
    for d in (textdir, trdir, comdir):
        os.makedirs(d, exist_ok=True)
    today = _dt.date.today().isoformat()

    # classify every text first: routing and the commentary index need it
    work_type, evidence, zh_titles = {}, {}, {}
    for entry in plan:
        if "root" not in entry["editions"]:
            continue
        bo = R[entry["editions"]["root"][0]["slug"]]
        zh = None
        for c in ("classical", "modern", "simplified"):
            if c in entry["editions"]:
                zh = R[entry["editions"][c][0]["slug"]]
                break
        wt, ev = classify_work(bo, zh)
        work_type[entry["text_key"]] = wt
        evidence[entry["text_key"]] = ev
        # Several rows leave the Chinese title columns blank and carry the
        # title only in the document filename, so fall back to that.
        zt = ""
        if zh:
            zt = (zh.get("name_short") or zh.get("name_long") or "").strip()
            if not zt:
                zt = parse_document_title(zh.get("doc_title", "")).get("title", "").strip()
        if not zt:
            zt = parse_document_title(bo.get("doc_title", "")).get("title", "").strip()
        if zt and wt in ("root-text", "treatise"):
            zh_titles[zt] = entry["text_key"]
        entry["_zh_title"] = zt

    written, report, inventory = [], [], []
    for entry in plan:
        tkey = entry["text_key"]
        eds = entry["editions"]
        if "root" not in eds:
        ...
        wt = work_type.get(tkey, "root-text")
        is_com = wt == "commentary"
        target = find_commentary_target(entry.get("_zh_title", ""), zh_titles) if is_com else None

        root_dir = comdir if is_com else textdir
        root_sub = "Commentaries" if is_com else "Text"
        root_name = "bo-%s.md" % stem
        root_path = os.path.join(root_dir, root_name)
        root_rel = "1-SOURCES/%s/%s" % (root_sub, root_name)

        def heading_bo(p):
            n = len(root_parts)
            return ("%s — %d/%d" % (bo_title, p["chapter"], n)) if n > 1 else bo_title

        total = sum(1 for p in root_parts for _s, t in p["segments"] if t.strip())
        fm = ["---",
              "title: %s" % yq(bo_title),
              "alt_titles:"]
        zh_first = None
        ...
        fm += ["author: %s" % yq(first["author_orig"]),
               ...
               "file_type: %s" % ("commentary" if is_com else "root-text"),
               "work_type: %s" % yq(wt),
               "work_type_evidence: %s" % yq(evidence.get(tkey, "")),
               "lang_tag: bo",
        ...
        if is_com:
            fm[-3:-3] = ([
                "commentary_on: %s" % yq(target or ""),
                "commentary_on_note: %s" % yq(
                    "" if target else
                    "root text not identified automatically — the title match was not exact; confirm by hand"),
            ])
        body = ["# %s" % bo_title, ""]
        if en_title:
            body += ["*%s*" % en_title, ""]
        body.append(render_text(root_parts, first, heading_bo, drop_empty=True))
        ...
        inventory.append({"text": tkey, "work_type": wt, "folder": root_sub,
                          "file": root_name, "tag": "bo", "register": "",
                          "segments": total, "root": "",
                          "commentary_on": target or "", "title": bo_title,
                          "title_en": en_title, "evidence": evidence.get(tkey, "")})

        # ---- root segment index, for transcluding from the translations.
        # Only segments that actually carry text are transcludable. An empty
        # Tibetan slot is the same placeholder the Chinese documents use, and
        # a transclusion pointing at one renders as nothing — the reverse of
        # the empty-Chinese case, and just as misleading.
        root_ids = {(p["chapter"], i)
                    for p in root_parts
                    for i, (_sid, txt) in enumerate(p["segments"], start=1)
                    if txt.strip()}
        ...
        for cls in sorted(k for k in eds if k != "root"):
            ...
            tdir = comdir if is_com else trdir
            tsub = "Commentaries" if is_com else "Translations"
            ...
            ztotal = sum(1 for p in parts for _s, t in p["segments"] if t.strip())
            zfm = ["---",
                   "title: %s" % yq(zh_title),
                   "alt_titles:"]
            if bo_title:
            ...
            zfm += ["author: %s" % yq(zr["author_orig"]),
                    ...
                    "file_type: %s" % ("commentary" if is_com else "translation"),
                    "work_type: %s" % yq(wt),
                    "lang_tag: %s" % yq(tag),
            ...
            written.append("1-SOURCES/%s/%s" % (tsub, tname))
            report.append({"text": tkey, "edition": cls, "file": tname,
                           "folder": tsub, "work_type": wt,
                           "segments": ztotal, "gaps": len(gaps),
                           "footnotes": bool(footnotes)})
            inventory.append({"text": tkey, "work_type": wt, "folder": tsub,
                              "file": tname, "tag": tag, "register": register,
                              "segments": ztotal, "root": root_rel,
                              "commentary_on": target or "", "title": zh_title})
    write_inventory(inventory, os.path.join(vault, "1-SOURCES", "INVENTORY.md"), today)
